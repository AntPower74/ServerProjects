...
with pdfplumber.open(pdf_path) as pdf:
    for page in pdf.pages:
        ...
        for y in sorted_y:
            ...
            # Special case for "Giorni di effettuazione"
            if line_text.startswith("Giorni di effettuazione"):
                # "Giorni di effettuazione" has length 23
                prefix_len = 23
                curr_len = 0
                giorni_words = []
                for w in line_words:
                ...
                # Match these with trips
                for w in giorni_words:
                    t = w['text'].strip()
                    x = w['x0']
                    # We might not have trips yet if this row appears at the top.
                    # Oh wait, "Giorni di effettuazione" is at the top of the page!
                    # If page_trips is empty, we create them!
                    trip_found = False
                    ...
                    if re.match(r'^(\d{1,2}[:.]\d{2}|[A-Z])$', t):
                        ...
                        if not trip_found:
                            page_trips.append({
                                ...
                                'giorni': 'Tutti' # fallback
                            })
                            
        for trip in page_trips:
            if len(trip['stops']) > 1:
                # Store stops and metadata
...
all_valid_stops = set()
for t in trips:
    all_valid_stops.update([k for k in t.keys() if not k.startswith('_')])
        ...
        validTrips.forEach((t, index) => {{
            ...
            card.innerHTML = badgeHtml + `
                <div class="trip-stations">
                    ...
                    <span class="giorni-badge">${{t.giorni}}</span>
                </div>
                <div class="trip-duration">
                    ${{durationHtml}}
                </div>
                <div class="trip-stations" style="text-align: right; align-items: flex-end;">
...
output_path = "/root/.gemini/antigravity-cli/brain/4cdfeb15-664f-4107-a0ca-620778a57ac3/RicercaCompleta.html"
