...
def get_riprese(pdf_path):
    ...
    for page in doc:
        ...
        for i in range(len(shifts)):
            ...
            # APPLY TEMPI ACCESSORI (-10m start, +10m end)
            if merged:
                merged[0] = (merged[0][0] - 10, merged[0][1])
                merged[-1] = (merged[-1][0], merged[-1][1] + 10)
                
            # Format with DOT '.' instead of COLON ':' for this old sheet
...
def main():
    ...
    if updates:
        ...
        print("Aggiornamento turni esistenti completato con i tempi accessori.")
