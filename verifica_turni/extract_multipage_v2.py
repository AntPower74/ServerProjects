...
def extract_all_optibus(pdf_path, valid_names):
    ...
    while i < n:
        ...
        # A page continues if IT ITSELF contains 'pagina seguente'
        combined_text = page.get_text().replace('\n', ' ')
        j = i
        
        while 'pagina seguente' in doc[j].get_text().lower():
            j += 1
            if j >= n: break
            combined_text += ' ' + doc[j].get_text().replace('\n', ' ')
        
        # Extract SIGN ON/OFF and PAUSAs from combined text
...
print(f"Extracted {len(all_shifts)} shifts")
print("Pe0170:", all_shifts.get('Pe0170'))
print("Pe0180:", all_shifts.get('Pe0180'))
print("Pt0010:", all_shifts.get('Pt0010'))
