...
pdf_out = "/home/antonio/verifica_turni/Comparazione turni 2025-2026/Dossier_Turni_2026_Layout_Comparativo_L138.pdf"

# A4 Landscape: 841.89 x 595.27 punti
...
law_body = ParagraphStyle('LawBody', fontName='Helvetica', fontSize=6.5, leading=8, textColor=colors.HexColor('#222222'))
...
print(f"Generazione {len(turni_dettagliati)} schede con layout a 3 sezioni...")
...
for idx, t in enumerate(turni_dettagliati):
    ...
    if code in ('To0280', 'To0660', 'Pi0140', 'Pi0200'):
        ...
        law_specifica = "<b>Applicazione Legge 138/1958 Art. 1, 2 e 7:</b> Il personale che effettua un orario settimanale pieno di 40 ore ripartite su 5 giornate lavorative ha diritto al doppio riposo compensativo settimanale consecutivo (Sabato e Domenica)."
    else:
        ...
        law_specifica = f"<b>Violazione Cartellino Azienda & Conformità ex L. 138/1958:</b><br/>" \
                        f"• <b>Art. 4 (Nastro & Interruzioni):</b> L'azienda impone un nastro illegittimo di <b>{n_att}</b> con stacco passivo non retribuito di oltre {int(risp_h)} ore. La proposta lo riconduce a <b>{p_nastro}</b> a norma di legge.<br/>" \
                        f"• <b>Art. 3 & 5 (Guida Continua & Pausa):</b> Guida ininterrotta sempre &le; 5h00 e pausa obbligatoria di almeno 30 minuti (frazionabile in 2x15m) garantita entro la 6ª ora di lavoro.<br/>" \
                        f"• <b>Art. 6 & 8 (Riposo Giornaliero & Residenza):</b> Garantito riposo minimo di 11h consecutive tra due turni con rientro obbligatorio nel deposito di residenza d'origine (<b>{dep}</b>)."

    # 1. HEADER GENERALE
    t_header = Table([
        [
            Paragraph(f"<b>ARRIVA ITALIA (ex SADEM) - SCHEDA COMPARATIVA DI RISTRUTTURAZIONE</b>", title_style),
    ...
    # Colonna Sinistra: Cartellino Azienda
    rows_orig = [
    ...
    # 3. SEZIONE INFERIORE (SOTTO: AUDIT & CONFORMITÀ LEGGE 14 FEBBRAIO 1958, N. 138)
    box_legge = [
        [
            Paragraph("<b>QUADRO DI CONFORMITÀ NORMATIVA - LEGGE 14 FEBBRAIO 1958, N. 138</b> (Orario Trasporto Pubblico Extraurbano Viaggiatori)", law_title)
        ],
        [
...
doc.build(elements)
print(f"✅ Documento finale generato con layout a 3 sezioni su {len(turni_dettagliati)} pagine!")
