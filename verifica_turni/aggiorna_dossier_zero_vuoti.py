...
print(f"Riformulazione descrizioni: eliminazione dicitura 'rientro a vuoto' e inserimento 'rientro in servizio di linea'...")
...
for idx, t in enumerate(tutti_turni):
    ...
    if is_ft:
        ...
        # PROPOSTA: SPECIFICA CHE IL RIENTRO È IN CORSA DI LINEA REGOLARE
        rows_prop = [
            ...
            [Paragraph("Part.", h_cell_p), Paragraph("Arr.", h_cell_p), Paragraph("Tipo", h_cell_p), Paragraph("Attività Proposta (Servizio di Linea Regolare)", h_cell_p)],
        ...
        rows_prop.append([Paragraph("-", cell_bold), Paragraph(p_fine, cell_bold), Paragraph("0002", cell_bold), Paragraph(f"Rientro in servizio di linea passeggeri al Deposito di {dep}", cell_bold)])
...
doc.build(elements)
print("✅ DOSSIER AGGIORNATO CON SUCCESSO! Chiarita la dicitura 'Rientro in servizio di linea' al posto di 'rientro a vuoto'.")
