    ...
    for pdf_path in pdfs:
        ...
        for page in doc:
            ...
            for idx, (shift_name, sy) in enumerate(sorted_shifts):
                ...
                # Extract locations for this shift (Y between sy-15 and sy-2)
                loc_words = []
                for w in words:
                    if sy - 15 < w[1] < sy - 2:
                ...
                for b_x0, b_x1, s, e in bars[1:]:
                    if not riprese or s - curr_end > 30:
                        riprese.append([curr_start, curr_end])
                        
                        # Find the best location text for the sosta based on curr_end_x
                        # The text is printed before the bar starts or ends
                        best_loc = ""
                        min_dist = 999
                        for lx, lt in grouped_locs:
                            dist = abs(lx - curr_end_x)
                            # Or if it's near the start of the next trip
                            dist2 = abs(lx - b_x0)
                            dist = min(dist, dist2)
...
for i, row in enumerate(data):
    ...
    if name in shifts_2025:
        ...
        # We don't need to clear, just write the new values
