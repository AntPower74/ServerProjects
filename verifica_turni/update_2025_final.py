...
def snap_time(graphical_mins, texts_with_x, target_x):
    ...
    # Optional sanity check: diff shouldn't be too huge (e.g. > 15 mins)
    if abs(diff) > 15:
        return graphical_mins
    ...
    for pdf_path in pdfs:
        ...
        for page in doc:
            ...
            for idx, (shift_name, sy) in enumerate(sorted_shifts):
                ...
                # Extract locations and times for snapping (Y between sy-15 and sy+25)
                loc_words = []
                time_texts_with_x = []
                for w in words:
                    if sy - 15 < w[1] < sy - 2:
                ...
                # Apply snapping to each bar BEFORE merging into riprese!
                snapped_bars = []
                for bx0, bx1, s, e in bars:
                    s_snap = snap_time(s, time_texts_with_x, bx0)
                    e_snap = snap_time(e, time_texts_with_x, bx1)
...
print(f'Extracted {len(shifts_2025)} total matched shifts from ALL 2025 PDFs with OCR Snapping.')
