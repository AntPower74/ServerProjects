...
print("Leggo i file HTML dei turni del SABATO...")
files = glob.glob('/home/antonio/verifica_turni/Turni settembre 2025 sab/VisualizzaTurnoStandard*')
shifts_data = {}
...
for fpath in files:
    ...
    for tr in tbody.find_all('tr'):
        ...
        if len(tds) > 4:
            s_name = tds[0].strip()
            # Clean shift name for matching (e.g. CA0070 -> Ca0070)
            if s_name:
            ...
            # Handle next day (e.g. 00:27 when start is 23:30)
            if end_m < start_m - 300: # heuristic for past midnight
                end_m += 1440
    ...
    if shift_name and shift_trips:
        ...
        # fix midnight crossover for start times
...
print(f"Estratti {len(shifts_data)} turni unici dai file HTML del SABATO.")
...
for i, row in enumerate(data):
    ...
    # standardize name for matching (e.g. Ca0020)
...
print(f"Sto per aggiornare {matched} turni SABATO nel foglio Google usando i dati originali del sito web...")
batch_size = 200
for i in range(0, len(updates), batch_size):
    ws.batch_update(updates[i:i+batch_size])

print("Aggiornamento SABATO completato con successo! (Precisione al 100%)")
