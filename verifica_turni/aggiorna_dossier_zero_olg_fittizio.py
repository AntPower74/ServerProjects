...
for idx, t in enumerate(tutti_turni):
    ...
    # PRINCIPIO RIGOROSO: OLG PROPOSTA = OLG EFFETTIVO ORIGINALE (NESSUN AUMENTO FITTIZIO)
    ...
    bg_diff_head = colors.HexColor('#CCE5FF') if (code == 'To0660' or is_lungo) else colors.HexColor('#E2E3E5')
    border_diff = colors.HexColor('#004085') if (code == 'To0660' or is_lungo) else colors.HexColor('#6C757D')
    ...
    t_diff.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), bg_diff_head), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#F4F9FF') if (code == 'To0660' or is_lungo) else colors.HexColor('#F8F9FA')), ('BOX', (0,0), (-1,-1), 1, border_diff), ('TOPPADDING', (0,0), (-1,-1), 1.5), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('LEFTPADDING', (0,0), (-1,-1), 4), ('RIGHTPADDING', (0,0), (-1,-1), 4)]))
    ...
    # DESTRA
    header_destra_bg = colors.HexColor('#006600') if (code == 'To0660' or is_lungo) else colors.HexColor('#004085')
    header_destra_sub_bg = colors.HexColor('#008800') if (code == 'To0660' or is_lungo) else colors.HexColor('#0056B3')
    
    rows_prop = [
        [Paragraph(f"<b>{'🟢 NOSTRA PROPOSTA RISTRUTTURATA' if (code == 'To0660' or is_lungo) else '🔵 NOSTRA VALUTAZIONE (CONFERMATO)'}</b><br/>Inizio: <b>{t['inizio']}</b> | Fine: <b>{p_fine}</b>", h_cell_p), Paragraph("", h_cell_p), Paragraph("", h_cell_p), Paragraph("", h_cell_p)],
    ...
    t_right.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0,-1), (-1,-1), colors.HexColor('#E8F5E9') if (code == 'To0660' or is_lungo) else colors.HexColor('#EBF3FA')),
        ('BOX', (0,-1), (-1,-1), 1, colors.HexColor('#006600') if (code == 'To0660' or is_lungo) else colors.HexColor('#004085')),
        ('GRID', (0,0), (-1,-2), 0.5, colors.HexColor('#DDDDDD')),
        ('ROWBACKGROUNDS', (0,2), (-1,-2), [colors.white, colors.HexColor('#F5FFF5') if (code == 'To0660' or is_lungo) else colors.HexColor('#F0F4F8')]),
        ('TOPPADDING', (0,0), (-1,-1), 0.8),
        ('BOTTOMPADDING', (0,0), (-1,-1), 0.8),
    ]))
...
doc.build(elements)
print("✅ DOSSIER UNIFICATO AGGIORNATO: OLG di To0660 impostato al suo valore REALE ed EFFETTIVO di 7h38m (zero aumenti fittizi)!")
