...
pdf_out = "/home/antonio/verifica_turni/Comparazione turni 2025-2026/Dossier_Turni_2026_DEPOSITO_SUSA_SALBERTRAND.pdf"
...
for pno, page in enumerate(doc_fitz):
    ...
    if turno.startswith('Su') or turno.startswith('Sa'):
        ...
        turni_susa.append({
            'turno': turno,
            'deposito': 'SUSA' if turno.startswith('Su') else 'SALBERTRAND',
...
print(f"Generazione DOSSIER DEDICATO SUSA & SALBERTRAND su {len(turni_susa)} turni...")

for idx, t in enumerate(turni_susa):
    code = t['turno']
    dep = t['deposito']
    n_att = t['nastro_str']
    o_att = t['olg_str']
    ...
    corse_prop_puntuali.append([t['inizio'], in_disp_fin, "Disp", f"Presa servizio & Controllo livelli a {dep} (1ª Ripresa)"])
    ...
    if vere_corse_linea:
        ...
        corse_prop_puntuali.append([in_disp_fin, "13:20", "0002", f"Servizio di linea regolare Alta Valle di Susa"])
    ...
    if is_lungo or is_sotto_paga:
        ...
        superamento_ccnl = f"<font color='#CC0000'><b>VIOLAZIONE GRAVE CCNL:</b> Il nastro azienda di {n_att} supera il tetto massimo di 12 ORE (+{int((n_val-12)*60)} min illegali).</font><br/>" if is_violazione_12h else ""
        ...
        box_diff_text = f"• <b>&Delta; Nastro:</b> 0h 00m (Confermato {n_att}) | " \
                        ...
                        f"• <b>STATO DEL TURNO:</b> <b>TURNO GIÀ CONFORME E REGOLARE.</b> Nastro inferiore alle 8h00 con corse continuative e rientro corretto nel Deposito di {dep}."
        law_specifica = "• <b>CCNL Autoferrotranvieri & L. 138/1958:</b> Turno già conforme. Nastro &le; 8h00, guida continua &le; 5h00, pause e riposo regolari."

    # HEADER
    t_header = Table([[Paragraph(f"<b>ARRIVA ITALIA (ex SADEM) - DOSSIER VALLE DI SUSA 2026</b>", title_style), Paragraph(f"<b>TURNO: <font color='#003366' size='11'>{code}</font> | Residenza: {dep}</b>", sub_style)]], colWidths=[405, 405])
    ...
    if idx < len(turni_susa) - 1:
        elements.append(PageBreak())

doc.build(elements)
print("✅ DOSSIER DEDICATO SUSA & SALBERTRAND GENERATO CON SUCCESSO SU 19 PAGINE!")
