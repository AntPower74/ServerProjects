#!/usr/bin/env python3
"""
RISOLUZIONE TOTALE E COMPLETA DEI TRASFERIMENTI GEOGRAFICI E DEGLI STACCHI RESIDUI
1. Elimina qualsiasi stacco residuo (> 60m) dai turni continui ottimizzati.
2. Garantisce che ogni turno inizi e finisca nello stesso deposito di residenza.
3. Se un turno arriva in un'altra città (es. Torino per bus di Caselle o Pinerolo),
   inserisce il trasferimento a vuoto reale (Trasf) con km e tempo di viaggio adeguati.
"""
...
for t_orig in turni_reali:
    ...
    if code in ['Pi0070', 'Bo3020']:
        turni_ottimizzati_perfetti.append(t)
        continue
    ...
    for a in att_raw:
        if a.get('linea') == 'Sosta' and a.get('durata_sosta_m', 0) >= 60:
            break # Trovato lo stacco passivo aziendale: fermiamo la prima parte qui!
        if 'chiusura' in a.get('descrizione', '').lower():
            continue
        att_prima_parte.append(a)
        
    # Se non c'erano soste esplicite ma c'è un gap temporale >= 60m tra due corse
    att_filtrate = []
    for i, a in enumerate(att_prima_parte):
        ...
        if i < len(att_prima_parte) - 1:
            ...
            if gap >= 60:
                break # Fermiamo al salto temporale
    ...
    # Inserimento corse e soste intermedie
    ...
    # Inserimento Sosta 30m certificata se nastro finale > 6h
    nastro_stimato = (curr_m - in_m) + 40 # Include sosta e chiusura
    has_30m = any((a.get('linea') == 'Sosta' or a.get('is_sosta_deposito')) and a.get('durata_sosta_m', 0) >= 30 for a in timeline)
    if not has_30m and nastro_stimato > 360:
    ...
    # Raggiungimento target nastro minimo 6h15 / 6h30 se turno compatto
    target_fin_m = in_m + 390 # Target 6h30
...
with open("/home/antonio/verifica_turni/web/turni_ottimizzati_completi.json", "w", encoding="utf-8") as f:
    json.dump(turni_ottimizzati_perfetti, f, ensure_ascii=False, indent=2)

print("✅ Risoluzione totale completata!")
