...
for t_orig in turni_reali:
    ...
    # Target Nastro: 6h30 per turni compattati, oppure nastro originario se già corto
    ...
    # Attività nella finestra di servizio
    ...
    # 2. Inserimento corse reali e pause/soste intermedie
    for i in range(len(att_in_window)):
        p_i = parse_m(att_in_window[i].get('partenza'))
        arr_i = parse_m(att_in_window[i].get('arrivo'))
    ...
    # 4. Raggiungimento del target nastro se inferiore
...
print("✅ Dataset ottimizzato rigenerato con timeline continua al 100%!")
