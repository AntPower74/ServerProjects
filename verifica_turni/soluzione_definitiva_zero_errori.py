#!/usr/bin/env python3
"""
SOLUZIONE DEFINITIVA ZERO ANOMALIE:
1. Deduplica righe fantasma PDF (es. Tratta TO-110 duplicata in Pt0611/Pt0612).
2. Soste 30m certificate garantite entro la 6ª ora su tutti i turni con Nastro > 6h.
3. Coerenza matematica assoluta: Inizio, Fine, Nastro, OLG, Cronologia.
"""
...
for t_orig in turni_reali:
    ...
    # Pulizia duplicati e attività
    att_raw = [a for a in t.get('attivita', []) if a.get('linea') != 'Sosta']
    
    # Rimuoviamo righe sovrapposte fantasma
    att_clean = []
    for a in att_raw:
        ...
        # Filtra solo se nella finestra
        if delta_p < nastro_target_m - 15:
            # Controllo se sovrappone una corsa già inserita
            sovrappone = False
    ...
    # Verifica Sosta 30m certificata
