...
for idx, t in enumerate(tutti_turni):
    ...
    if is_ft:
        ...
        box_diff_text = f"• <b>VERSIONE 1 (Azienda):</b> Micro-spezzone isolato di sole {o_att} pagate | " \
                        f"• <b>VERSIONE 2 (Nostra Base):</b> Turno isolato a paga minima | " \
                        f"• <b>VERSIONE 3 (Aggancio FT):</b> <b>ACCORPATO AL TURNO {target_turno_aggancio}</b> (Paga piena a {nuovo_olg_ft}, nastro compatto senza buchi)."
        law_specifica = "• <b>Ottimizzazione Contrattuale Turni FT:</b> L'aggancio delle corse scolastiche bis FT al turno di linea garantisce la retribuzione di una giornata piena conforme al CCNL e alla L. 138/1958."
        
        t_header = Table([[Paragraph(f"<b>ARRIVA ITALIA (ex SADEM) - SCHEDA COMPARATIVA A 3 VERSIONI</b>", title_style), Paragraph(f"<b>TURNO: <font color='#003366' size='11'>{code}</font> | Residenza: {dep}</b>", sub_style)]], colWidths=[405, 405])
        t_header.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('BOTTOMPADDING', (0,0), (-1,-1), 1)]))
        elements.append(t_header)
        elements.append(Spacer(1, 1))

        t_diff = Table([[Paragraph(f"<b>📊 QUADRO COMPARATIVO A 3 VERSIONI - {stato_header}</b>", diff_title)], [Paragraph(box_diff_text, diff_body)]], colWidths=[810])
        t_diff.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#FFF3CD')), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#FFFDF5')), ('BOX', (0,0), (-1,-1), 1, colors.HexColor('#856404')), ('TOPPADDING', (0,0), (-1,-1), 1.5), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('LEFTPADDING', (0,0), (-1,-1), 4), ('RIGHTPADDING', (0,0), (-1,-1), 4)]))
        elements.append(t_diff)
        elements.append(Spacer(1, 2))

        rows_c1 = [[Paragraph(f"<b>🔴 1. ATTUALE AZIENDA</b><br/>Inizio: <b>{t['inizio']}</b> | Fine: <b>{t['fine']}</b><br/>Nastro: <b>{n_att}</b> | OLG: <font color='#CC0000'><b>{o_att}</b></font>", h_cell_o), Paragraph("", h_cell_o)], [Paragraph("Orario", h_cell_o), Paragraph("Attività Aziendale", h_cell_o)], [Paragraph(t['inizio'], cell_bold), Paragraph("Presa servizio & Controllo", cell_text)]]
        if t['corse_raw']:
            for r in t['corse_raw']: rows_c1.append([Paragraph(f"{r[0]}-{r[1]}", cell_bold), Paragraph(r[3], cell_text)])
        ...
        t_c1.setStyle(TableStyle([('SPAN', (0,0), (1,0)), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#990000')), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#B30000')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#DDDDDD')), ('ROWBACKGROUNDS', (0,2), (-1,-1), [colors.white, colors.HexColor('#FFF5F5')]), ('TOPPADDING', (0,0), (-1,-1), 1.2), ('BOTTOMPADDING', (0,0), (-1,-1), 1.2)]))

        rows_c2 = [[Paragraph(f"<b>🔵 2. NOSTRA MODIFICA BASE</b><br/>Inizio: <b>{t['inizio']}</b> | Fine: <b>{t['fine']}</b><br/>Nastro: <b>{n_att}</b> | OLG: <b>{o_att}</b>", h_cell_o), Paragraph("", h_cell_o)], [Paragraph("Orario", h_cell_o), Paragraph("Attività Turno Stand-Alone", h_cell_o)], [Paragraph(t['inizio'], cell_bold), Paragraph(f"Presa servizio regolare a {dep}", cell_text)], [Paragraph(f"{t['inizio']}-{t['fine']}", cell_bold), Paragraph("Copertura corsa bis scolastica", cell_text)], [Paragraph(t['fine'], cell_bold), Paragraph("Chiusura turno singolo", cell_text)], [Paragraph("Nota", cell_bold), Paragraph("Mantenuto isolato", cell_text)]]
        t_c2 = Table(rows_c2, colWidths=[55, 215])
        t_c2.setStyle(TableStyle([('SPAN', (0,0), (1,0)), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#004085')), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#0056B3')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#DDDDDD')), ('ROWBACKGROUNDS', (0,2), (-1,-1), [colors.white, colors.HexColor('#F0F4F8')]), ('TOPPADDING', (0,0), (-1,-1), 1.2), ('BOTTOMPADDING', (0,0), (-1,-1), 1.2)]))

        rows_c3 = [[Paragraph(f"<b>🟢 3. MODIFICA CON AGGANCIO FT</b><br/>Inizio: <b>{t['inizio']}</b> | Fine: <b>14:00</b><br/>Nuovo Nastro: <b>{nuovo_nastro_ft}</b> | OLG: <font color='#008800'><b>{nuovo_olg_ft}</b></font>", h_cell_ft), Paragraph("", h_cell_ft)], [Paragraph("Orario", h_cell_ft), Paragraph(f"Attività Accorpata a {target_turno_aggancio}", h_cell_ft)], [Paragraph(t['inizio'], cell_bold), Paragraph(f"Presa servizio & Corsa Bis FT", cell_text)], [Paragraph("08:35", cell_bold), Paragraph(f"Raccordo su corsa di linea ordinaria", cell_text)], [Paragraph("In linea", cell_bold), Paragraph("Pausa regolamentare 30m L. 138/58", cell_bold)], [Paragraph("14:00", cell_bold), Paragraph(f"Rientro in linea regolare al Deposito di {dep}", cell_text)], [Paragraph("Esito", cell_bold), Paragraph("<b>Paga piena giornaliera raggiunta!</b>", cell_bold)]]
        t_c3 = Table(rows_c3, colWidths=[55, 215])
        t_c3.setStyle(TableStyle([('SPAN', (0,0), (1,0)), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#006600')), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#008800')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#DDDDDD')), ('ROWBACKGROUNDS', (0,2), (-1,-1), [colors.white, colors.HexColor('#F5FFF5')]), ('TOPPADDING', (0,0), (-1,-1), 1.2), ('BOTTOMPADDING', (0,0), (-1,-1), 1.2)]))
        ...
        t_law.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#FFF3CD')), ('BOX', (0,0), (-1,-1), 0.5, colors.HexColor('#856404')), ('TOPPADDING', (0,0), (-1,-1), 1.5), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('LEFTPADDING', (0,0), (-1,-1), 4), ('RIGHTPADDING', (0,0), (-1,-1), 4)]))
        ...
        if is_40h:
            ...
            box_diff_text = f"• <b>&Delta; Nastro (Tempo risparmiato):</b> <font color='#006600'><b>{diff_nastro_str} 👍</b></font> (Da {n_att} a {p_nastro}) | " \
                            ...
                            f"• <b>&Delta; Riprese:</b> {diff_rip_str}<br/>" \
                            f"• <b>DIFFERENZE OPERATIVE:</b> Eliminato stacco passivo diurno di oltre {int(risp_h)} ore | Rientro garantito in corsa di linea al Deposito di {dep}."
            superamento_ccnl = ""
            if n_val > 12.0:
        ...
        # A DESTRA: NOSTRA PROPOSTA CON CORSE REALI E RIENTRO IN SERVIZIO COMMERCIALE
        rows_prop = [
            ...
            [Paragraph("Part.", h_cell_p), Paragraph("Arr.", h_cell_p), Paragraph("Tipo", h_cell_p), Paragraph("Articolazione Servizio di Linea Regolare", h_cell_p)],
            [Paragraph(t['inizio'], cell_bold), Paragraph("-", cell_bold), Paragraph("Disp", cell_bold), Paragraph(f"Presa servizio & Controllo livelli al Deposito di {dep}", cell_text)]
        ]
        
        if code == "Ba3520":
            # Dettaglio specifico Ba3520
            rows_prop.append([Paragraph("06:40", cell_bold), Paragraph("07:10", cell_bold), Paragraph("000280", cell_bold), Paragraph("BARGE -> OSASCO Ponte Chisone", cell_text)])
            rows_prop.append([Paragraph("07:10", cell_bold), Paragraph("07:24", cell_bold), Paragraph("000275", cell_bold), Paragraph("OSASCO -> Villar Perosa SKF", cell_text)])
            rows_prop.append([Paragraph("07:45", cell_bold), Paragraph("08:10", cell_bold), Paragraph("000281", cell_bold), Paragraph("Villar Perosa -> Pinerolo Centro Studi", cell_text)])
            rows_prop.append([Paragraph("08:30", cell_bold), Paragraph("09:00", cell_bold), Paragraph("PAUSA", cell_bold), Paragraph("Pausa regolamentare 30m ex Art. 5 L. 138/58", cell_bold)])
            rows_prop.append([Paragraph("09:15", cell_bold), Paragraph("10:15", cell_bold), Paragraph("000280", cell_bold), Paragraph("Pinerolo -> Corsa di linea commerciale per Barge", cell_text)])
            rows_prop.append([Paragraph("10:15", cell_bold), Paragraph("13:30", cell_bold), Paragraph("000280", cell_bold), Paragraph("Servizio di linea integrato direttrice Barge / Saluzzo", cell_text)])
            rows_prop.append([Paragraph("-", cell_bold), Paragraph("13:45", cell_bold), Paragraph("Disp", cell_bold), Paragraph("Rientro a BARGE PARCHEGGIO & Chiusura servizio compatto", cell_text)])
        elif t['corse_raw']:
            for r in t['corse_raw'][:5]:
            ...
            rows_prop.append([Paragraph("-", cell_bold), Paragraph(p_fine, cell_bold), Paragraph("0002", cell_bold), Paragraph(f"Rientro in servizio di linea passeggeri al Deposito di {dep}", cell_text)])
        else:
            rows_prop.append([Paragraph(t['inizio'], cell_bold), Paragraph(p_fine, cell_bold), Paragraph("0002", cell_bold), Paragraph(f"Copertura continuativa linea {dep}", cell_text)])
            rows_prop.append([Paragraph("In linea", cell_bold), Paragraph("30m", cell_bold), Paragraph("PAUSA", cell_bold), Paragraph("Pausa obbligatoria &ge; 30m entro 6h (Art. 5 L. 138/58)", cell_bold)])
            rows_prop.append([Paragraph("-", cell_bold), Paragraph(p_fine, cell_bold), Paragraph("0002", cell_bold), Paragraph(f"Rientro in linea al Deposito di {dep}", cell_text)])
...
doc.build(elements)
print("✅ DOSSIER UNIFICATO AGGIORNATO CON SUCCESSO! Dettaglio corse reali e rientri commerciali integrati.")
