...
for idx, t in enumerate(tutti_turni):
    ...
    # Garanzia Retributiva Minima CCNL: se un turno ordinario a tempo pieno è sotto 6h30, la proposta applica la garanzia a 6h30 piene (eccetto i part-time FT)
    if is_ft:
        p_olg = f"{o_att}".replace(',', 'h ') + "m (Part-Time)"
    elif is_40h:
        p_olg = "8h 00m (40h Sett.)"
    elif o_val < 6.5:
        p_olg = "6h 30m (Garanzia CCNL)"
    else:
...
doc.build(elements)
print("✅ DOSSIER UNIFICATO AGGIORNATO: Garanzia Retributiva CCNL applicata su tutti i turni con OLG < 6h30!")
