...
for t_orig in turni_reali:
    ...
    # Filtra solo corse nella finestra senza sovrapposizioni
    att_clean = []
    for a in att_raw:
    ...
    # 2. Inserimento di tutte le corse e dei loro gap naturali
    ...
    # 3. Se il turno supera 6 ore e non ha ancora una sosta da 30m, la inseriamo al termine delle corse
    has_30m = any((a.get('linea') == 'Sosta' or a.get('is_sosta_deposito')) and a.get('durata_sosta_m', 0) >= 30 for a in timeline)
    if not has_30m and nastro_target_m > 360:
    ...
    # 4. Raggiungimento target nastro
...
print("✅ Rigenerazione pulita completata!")
