...
turni_migliorati_list = [t for t in tutti_turni if t['turno'] in mappa_sinergie_complete]
turni_confermati_list = [t for t in tutti_turni if t['turno'] not in mappa_sinergie_complete]
...
def build_pdf(lista_turni, filename, is_migl_pdf, title_prefix=""):
    ...
    for idx, t in enumerate(lista_turni):
        ...
        has_sinergia = code in mappa_sinergie_complete
        
        if has_sinergia:
            stato_header = "🟢 RISTRUTTURATO CON SINERGIA OPERATIVA TORINO <-> VALLI"
            box_diff_text = f"• <b>SINERGIA OPERATIVA:</b> {mappa_sinergie_complete[code]}<br/>" \
                            f"• <b>Nastro:</b> {p_nastro} | • <b>OLG (Paga):</b> {p_olg} | • <b>Riprese:</b> 1 (Continuo)"
            nota_cambio_turno = f"■ <b>SINERGIA OPERATIVA:</b> {mappa_sinergie_complete[code]}"
        else:
        ...
        bg_box = colors.HexColor('#CCE5FF') if has_sinergia else colors.HexColor('#E2E3E5')
        border_box = colors.HexColor('#004085') if has_sinergia else colors.HexColor('#6C757D')
        t_diff = Table([[Paragraph(f"<b>📊 QUADRO DI SINTESI - {stato_header}</b>", diff_title)], [Paragraph(box_diff_text, diff_body)]], colWidths=[816])
        t_diff.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), bg_box), ('BACKGROUND', (0,1), (-1,1), colors.HexColor('#F4F9FF') if has_sinergia else colors.HexColor('#F8F9FA')), ('BOX', (0,0), (-1,-1), 1, border_box), ('TOPPADDING', (0,0), (-1,-1), 1.5), ('BOTTOMPADDING', (0,0), (-1,-1), 1.5), ('LEFTPADDING', (0,0), (-1,-1), 4), ('RIGHTPADDING', (0,0), (-1,-1), 4)]))
        ...
        # DESTRA
        h_destra_bg = colors.HexColor('#006600') if has_sinergia else colors.HexColor('#004085')
        h_destra_sub = colors.HexColor('#008800') if has_sinergia else colors.HexColor('#0056B3')
        rows_prop = [
            [Paragraph(f"<b>{'🟢 NOSTRA PROPOSTA RISTRUTTURATA' if has_sinergia else '🔵 NOSTRA SCHEDA COMPARATIVA'}</b><br/>Inizio: <b>{t['inizio']}</b> | Fine: <b>{p_fine}</b>", h_cell_p), Paragraph("", h_cell_p), Paragraph("", h_cell_p), Paragraph("", h_cell_p), Paragraph("", h_cell_p)],
        ...
        t_right.setStyle(TableStyle([('SPAN', (0,0), (4,0)), ('SPAN', (0,-1), (4,-1)), ('BACKGROUND', (0,0), (-1,0), h_destra_bg), ('BACKGROUND', (0,1), (-1,1), h_destra_sub), ('BACKGROUND', (0,-1), (-1,-1), colors.HexColor('#E8F5E9') if has_sinergia else colors.HexColor('#EBF3FA')), ('BOX', (0,-1), (-1,-1), 1, colors.HexColor('#006600') if has_sinergia else colors.HexColor('#004085')), ('GRID', (0,0), (-1,-2), 0.5, colors.HexColor('#DDDDDD')), ('ROWBACKGROUNDS', (0,2), (-1,-2), [colors.white, colors.HexColor('#F5FFF5') if has_sinergia else colors.HexColor('#F0F4F8')]), ('TOPPADDING', (0,0), (-1,-1), 0.8), ('BOTTOMPADDING', (0,0), (-1,-1), 0.8)]))
...
build_pdf(turni_migliorati_list, pdf_migl_out, is_migl_pdf=True, title_prefix="DOSSIER TURNI MIGLIORATI (SINERGIE TORINO-VALLI)")
...
print(f"✅ GENERATI TUTTI I PDF AGGIORNATI CON LE SINERGIE DI TORINO:\n1. {pdf_migl_out} ({len(turni_migliorati_list)} Pagine: Sinergie Torino-Valli)\n2. {pdf_conf_out} ({len(turni_confermati_list)} Pagine: Turni Confermati)\n3. {pdf_unif_out} ({len(tutti_turni)} Pagine: Dossier Completo)")
