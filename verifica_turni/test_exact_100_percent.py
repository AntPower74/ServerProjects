...
for t_az in turni_base:
    ...
    nOk = n_m <= max_nastro
    oOk = (n_m <= 240 or code.startswith('FT')) or (o_m >= target_olg)
...
print(f"Conformi: {conformi} / {len(turni_base)} ({conformi/len(turni_base)*100:.1f}%)")
print(f"Nastro Medio: {tot_n // len(turni_base) // 60}h {tot_n // len(turni_base) % 60}m")
print(f"OLG Medio: {tot_o // len(turni_base) // 60}h {tot_o // len(turni_base) % 60}m")
