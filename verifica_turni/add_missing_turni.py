...
def main():
    ...
    for row in data:
        if row:
            t = row[0].strip()
            if t: sheet_names.add(t)
            
    missing_names = []
    for k in all_riprese.keys():
        if k not in sheet_names:
            missing_names.append(k)
            
    print(f"Turni da aggiornare: {len(all_riprese)}")
    print(f"Turni mancanti che verranno aggiunti: {len(missing_names)}")
    print(missing_names)
    ...
    if missing_names:
        ...
        for m_name in missing_names:
            ...
            # Construct a row. Col A is m_name.
            row = [m_name, '', '', '', '']
            # We need to fill columns up to V (22 columns).
            # F(5)=start1, G(6)=end1, H(7)=sosta, I(8)=start2, J(9)=end2...
            for _ in range(17):
                row.append('')
