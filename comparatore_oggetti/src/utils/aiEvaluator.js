...
export function valutaOggettoUniversale(testoAnnuncio, prezzoManuale) {
  ...
  for (const [andKey, andData] of Object.entries(ANDROID_MODELS)) {
    if (clean.includes(andKey)) {
      ...
      if (prezzoRichiesto) {
        ...
        discountText = sconto > 0
          ? ` (Chiede ${prezzoRichiesto}€ ➔ Chiedi uno sconto del ${sconto}%)`
          : ` (Chiede ${prezzoRichiesto}€: prezzo già in linea col valore stimato, puoi acquistarlo a questa cifra)`
      }
  ...
  if (prezzoRichiesto) {
    const sconto = Math.round(((prezzoRichiesto - targetVal) / prezzoRichiesto) * 100)
    consiglioExtra += sconto > 0
      ? ` Il venditore chiede ${prezzoRichiesto}€: offrendo ${targetVal}€ chiedi uno sconto del ${sconto}%.`
      : ` Il venditore chiede ${prezzoRichiesto}€: è già un prezzo in linea col valore stimato, puoi acquistarlo a questa cifra.`
  }
