...
export const ANDROID_MODELS = {
  ...
  'galaxy a54': { name: 'Samsung Galaxy A54 5G', screen: '6.4" Super AMOLED 120Hz', baseResale: 180, retail: '499€', tier: 'Top Fascia Media Samsung' },
  'galaxy a53': { name: 'Samsung Galaxy A53 5G', screen: '6.5" Super AMOLED 120Hz', baseResale: 150, retail: '449€', tier: 'Fascia Media Samsung' },
  'galaxy a52': { name: 'Samsung Galaxy A52 / A52s 5G', screen: '6.5" Super AMOLED 120Hz', baseResale: 125, retail: '399€', tier: 'Best Seller Storico Samsung' },
  'galaxy a34': { name: 'Samsung Galaxy A34 5G', screen: '6.6" Super AMOLED 120Hz', baseResale: 140, retail: '399€', tier: 'Fascia Media Samsung' },
  'galaxy a33': { name: 'Samsung Galaxy A33 5G', screen: '6.4" Super AMOLED 90Hz', baseResale: 115, retail: '349€', tier: 'Fascia Media Economica' },
  'galaxy a14': { name: 'Samsung Galaxy A14 / A14 5G', screen: '6.6" PLS LCD', baseResale: 80, retail: '199€', tier: 'Entry Level Samsung' },
  'galaxy a13': { name: 'Samsung Galaxy A13', screen: '6.6" PLS LCD', baseResale: 70, retail: '179€', tier: 'Entry Level Economico' },
  'galaxy a12': { name: 'Samsung Galaxy A12', screen: '6.5" PLS LCD', baseResale: 60, retail: '169€', tier: 'Entry Level Economico' },
  ...
  'oppo a79': { name: 'OPPO A79 5G', screen: '6.72" FHD+ 90Hz', baseResale: 110, retail: '299€', tier: 'Fascia Media 5G OPPO' },
  'oppo a78': { name: 'OPPO A78 (128/256 GB)', screen: '6.43" AMOLED 90Hz', baseResale: 95, retail: '269€', tier: 'Fascia Media OPPO' },
  'oppo a77': { name: 'OPPO A77 5G / 4G (128 GB)', screen: '6.56" HD+ 90Hz', baseResale: 85, retail: '229€', tier: 'Entry Level / Fascia Media OPPO' },
  'a77': { name: 'OPPO A77 5G / 4G (128 GB)', screen: '6.56" HD+ 90Hz', baseResale: 85, retail: '229€', tier: 'Entry Level / Fascia Media OPPO' },
  'oppo a74': { name: 'OPPO A74 (128 GB)', screen: '6.43" AMOLED', baseResale: 65, retail: '299€', tier: 'Fascia Economica OPPO' },
  'a74': { name: 'OPPO A74 (128 GB)', screen: '6.43" AMOLED', baseResale: 65, retail: '299€', tier: 'Fascia Economica OPPO' },
  'oppo a72': { name: 'OPPO A72 (128 GB)', screen: '6.5" IPS', baseResale: 60, retail: '279€', tier: 'Entry Level Economico' },
  'oppo a57': { name: 'OPPO A57s / A57', screen: '6.56" IPS', baseResale: 70, retail: '189€', tier: 'Entry Level OPPO' },
  'oppo a54': { name: 'OPPO A54 5G', screen: '6.5" IPS 90Hz', baseResale: 75, retail: '249€', tier: 'Entry Level 5G OPPO' },
  'oppo a53': { name: 'OPPO A53s / A53', screen: '6.5" 90Hz', baseResale: 55, retail: '199€', tier: 'Entry Level Economico' },
  'oppo a52': { name: 'OPPO A52', screen: '6.5" FHD+', baseResale: 50, retail: '199€', tier: 'Entry Level Economico' },
  'oppo a16': { name: 'OPPO A16 / A16s', screen: '6.52" IPS', baseResale: 55, retail: '159€', tier: 'Entry Level Economico' },
  'oppo a17': { name: 'OPPO A17', screen: '6.56" IPS', baseResale: 60, retail: '169€', tier: 'Entry Level Economico' },
  'oppo a96': { name: 'OPPO A96', screen: '6.59" 90Hz', baseResale: 105, retail: '299€', tier: 'Fascia Media OPPO' },
  'oppo a94': { name: 'OPPO A94 5G', screen: '6.43" AMOLED', baseResale: 95, retail: '349€', tier: 'Fascia Media OPPO' },
  'oppo reno': { name: 'OPPO Reno Series', screen: 'AMOLED', baseResale: 130, retail: '499€', tier: 'Fascia Media Design' },
  'oppo a': { name: 'OPPO Serie A', screen: 'IPS / AMOLED', baseResale: 65, retail: '229€', tier: 'Fascia Economica OPPO' },
  'oppo': { name: 'OPPO Smartphone', screen: 'AMOLED / IPS', baseResale: 75, retail: '299€', tier: 'Fascia Media Android' },
 ...
 */
export function valutaOggettoUniversale(testoAnnuncio) {
  const clean = (testoAnnuncio || '').toLowerCase().trim()
  if (!clean) return null

  // 1. Estrazione Prezzo Richiesto dall'annuncio (se specificato)
  ...
  for (const [andKey, andData] of Object.entries(ANDROID_MODELS)) {
    if (clean.includes(andKey)) {
      ...
      const targetVal = Math.round(resaleVal * 0.52) // Regola ~50%
      const maxVal = Math.round(targetVal * 1.18)
  ...
  for (const [mangaKey, mangaData] of Object.entries(MANGA_DATABASE)) {
    if (clean.includes(mangaKey)) {
      ...
      const targetVal = Math.max(15, Math.round(resaleTotale * 0.48))
      const maxVal = Math.round(targetVal * 1.20)
  ...
  if (clean.includes('iphone') || clean.includes('apple')) {
    ...
    if (matchedKey) {
      ...
      const targetVal = Math.round(resaleVal * 0.62)
      const maxVal = Math.round(targetVal * 1.15)
      const profit = resaleVal - targetVal
      const roi = Math.round((profit / targetVal) * 100)
    ...
    for (const [kw, g] of Object.entries(GAMES_CATALOG)) {
      if (clean.includes(kw) && !clean.includes(`${kw}: venduto`)) {
        if (!giochiInclusi.some(x => x.name === g.name)) {
          ...
          totalBundleResale += g.val
        }
      }
    }

    const consoleName = isOled ? 'Nintendo Switch OLED' : isLite ? 'Nintendo Switch Lite' : 'Nintendo Switch (V1/V2 Base)'
    const targetVal = Math.round(totalBundleResale * 0.55)
    const maxVal = Math.round(targetVal * 1.18)
    const profit = totalBundleResale - targetVal
    const roi = Math.round((profit / targetVal) * 100)

    return {
      titoloRilevato: `Bundle ${consoleName} + ${giochiInclusi.length} Giochi & Accessori`,
      rawText: testoAnnuncio,
      schedaOggetto: {
        tipologia: `${consoleName} con dock, cavi originali, scatola + ${extraItems.length} accessori extra + ${giochiInclusi.length} giochi su cartuccia`,
        fasciaMercato: 'Top di Gamma Console Ibrida Nintendo',
        prezzoNuovo: 'Valore a nuovo del pacchetto intero: ~450€ – 500€',
        prezzoUsatoDettaglio: `~${totalBundleResale}€ (smembrando console, joy-con extra e cartucce su Vinted/Subito)`
      },
      strategiaFlipping: [
        { voce: 'Offerta Target per il Blocco', valore: `${targetVal}€`, badge: 'DA INVIARE', highlight: true },
        { voce: 'Tetto Massimo da non superare', valore: `${maxVal}€`, nota: 'Oltre questa cifra il margine scende' },
        { voce: 'Incasso Reale Totale (Smembrato)', valore: `~${totalBundleResale}€`, nota: `Console 140€ + Joycon 40€ + Giochi ${giochiInclusi.reduce((a,b)=>a+b.val,0)}€` },
        { voce: 'Profitto Netto Stimato', valore: `+${profit}€`, nota: `ROI: +${roi}%`, isProfit: true }
      ],
      liquidita: {
        livello: 'Massima (Liquidità Record)',
        benchmark: 'La Switch e i suoi accessori originali sono tra gli oggetti più facili da smembrare e rivendere a Torino',
        consiglioOperativo: `Strategia vincente: Acquista l'intero blocco a ${targetVal}€–135€. Tieni la console a 140€ su Subito a mano a Torino e metti subito i Joy-Con rosa/verdi a 42€ e i giochi su Vinted. Fai +${profit}€ puliti in 3 giorni!`
      },
      controlliDalVivo: [
      ...
      roi,
      scriptTrattativa: {
        subito: `Ciao! Ho visto il tuo annuncio del bundle Nintendo Switch con la scatola e gli accessori. Visto che prenderei tutto il blocco completo subito, posso offrirti ${targetVal}€ e venire a fare ritiro a mano di persona oggi stesso in contanti a Torino. Fammi sapere se per te può andare bene!`,
        whatsapp: `Buongiorno! Ti scrivo per il bundle Nintendo Switch. Sono di Torino e posso fare ritiro a mano oggi a ${targetVal}€ in contanti sul posto. Fammi sapere dove ci possiamo incontrare, grazie!`
      }
    }
  }
  ...
  const targetVal = Math.max(10, Math.round(resaleVal * targetMargin))
  const maxVal = Math.round(targetVal * 1.20)
  const profit = resaleVal - targetVal
  const roi = Math.round((profit / targetVal) * 100)
